...
import {
  PenSquare, Trash2, CheckCircle, XCircle,
  Eye, Sparkles, Plus, Filter,
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/posts/new" className="btn-amber">
            ...
            <tbody className="divide-y divide-ink-50">
              {posts.map(p => (
                <tr key={p.id} className="hover:bg-ink-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-ink-600 whitespace-nowrap">{p.author}</td>
                      ...
                      {p.status === 'pending' && (
                        <>
                          <button onClick={() => handlePublish(p.id)}
                            className="p-1.5 rounded-lg text-green-600 hover:bg-green-50 transition-colors" title="Publish">
                            <CheckCircle size={15} />
                          </button>
