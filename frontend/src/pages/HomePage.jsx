...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setFeatured(blogs.data.posts);
      setLatestNews(news.data.posts);
    ...
    <div>
      ...
      <section className="bg-ink-900 text-white py-20 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-bold leading-tight mb-5">
            Stories that <em className="text-amber-400 not-italic">inform</em><br/>
            and inspire.
          </h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-14 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2">
          ...
          {featured.length === 0 ? (
            <EmptyState label="No blog posts yet." />
          ) : (
            <div className="space-y-6">
              {featured[0] && <PostCard post={featured[0]} featured />}
              <div className="card divide-y divide-ink-100">
                {featured.slice(1).map(p => <PostCard key={p.id} post={p} />)}
              </div>
            </div>
          )}
        </div>
        ...
        <aside>
          ...
          {latestNews.length === 0 ? (
            ...
            <div className="card divide-y divide-ink-100">
              {latestNews.map(p => <PostCard key={p.id} post={p} />)}
            </div>
          )}
        </aside>
      </div>
...
function EmptyState({ label }) {
  return (
    <div className="card p-10 text-center text-ink-400 font-sans text-sm">{label}</div>
  );
}
...
function HeroSkeleton() {
  return (
    <div className="animate-pulse">
      ...
      <div className="max-w-6xl mx-auto px-4 py-14 grid grid-cols-1 lg:grid-cols-3 gap-10">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-40 bg-ink-100 rounded-2xl" />
        ))}
      </div>
    </div>
  );
