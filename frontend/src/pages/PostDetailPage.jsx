...
import {
  Eye, MessageCircle, Calendar, User, Sparkles,
  Trash2, CheckCircle, XCircle, Send, ArrowLeft,
} from 'lucide-react';
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <button onClick={() => navigate(-1)}
        className="btn-ghost mb-8 -ml-2 text-sm text-ink-500">
      ...
      <div className="flex flex-wrap gap-2 items-center mb-4">
        ...
        {post.is_ai_generated && (
          <span className="badge badge-green"><Sparkles size={10} className="mr-0.5" /> AI Generated</span>
        )}
      </div>

      {/* Title */}
      <h1 className="font-serif text-3xl md:text-4xl font-bold text-ink-900 leading-tight mb-6">
        {post.title}
      </h1>
      ...
      <div className="flex items-center gap-4 pb-6 mb-8 border-b border-ink-100">
        ...
        <div>
          <p className="font-sans font-medium text-ink-900 text-sm">{post.author}</p>
          <div className="flex items-center gap-3 text-xs text-ink-400 font-sans mt-0.5">
            <span className="flex items-center gap-1"><Calendar size={11} />{format(published, 'MMM d, yyyy')}</span>
      ...
      {post.tags?.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-12">
          {post.tags.map(t => (
            <span key={t.slug} className="badge badge-gray">#{t.name}</span>
          ))}
        </div>
      )}
      ...
      <section>
        ...
        <div className="space-y-4 mb-8">
          {comments.length === 0 && (
            <p className="text-ink-400 font-sans text-sm py-6 text-center">
              No comments yet. Be the first!
            </p>
          )}
          {comments.map(c => (
            <div key={c.id}
              className={`card p-4 ${c.status === 'pending' ? 'border-amber-200 bg-amber-50' : ''} ${c.status === 'rejected' ? 'opacity-50' : ''}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-sans text-sm font-medium text-ink-900">{c.author}</span>
                  <span className="text-xs text-ink-400 font-sans">
                    {formatDistanceToNow(new Date(c.created_at), { addSuffix: true })}
                  </span>
                  {c.status !== 'approved' && (
                    <span className={`badge ${c.status === 'pending' ? 'badge-amber' : 'badge-red'}`}>
                      {c.status}
                    </span>
                  )}
                </div>
